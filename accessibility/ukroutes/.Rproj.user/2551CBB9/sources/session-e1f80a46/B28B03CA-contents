...
setnames(osgsl_new, old = "easting", new = "EASTING") # Rename variable for consistency
...
temp <- accesspoints[accesspoints$size >= 0.5,] # Subset those which meet the size threshold (0.5ha) - n = 18759 (68%)
temp$size <- NULL # Drop variables not required
write_parquet(osgsl_new, "../data/processed/osgsl/osgsl_doorstop.parquet") # Save

# Local green space
temp <- accesspoints[accesspoints$size >= 2,] # Subset those which meet the size threshold (2ha) - n = 10653 (39%)
temp$size <- NULL # Drop variables not required
write_parquet(osgsl_new, "../data/processed/osgsl/osgsl_local.parquet") # Save
...
temp$size <- NULL # Drop variables not required
write_parquet(osgsl_new, "../data/processed/osgsl/osgsl_neighbourhood.parquet") # Save

# Wider green space
temp <- accesspoints[accesspoints$size >= 20,] # Subset those which meet the size threshold (20ha) - n = 2486 (9%)
temp$size <- NULL # Drop variables not required
write_parquet(osgsl_new, "../data/processed/osgsl/osgsl_wider.parquet") # Save

# District green space
temp <- accesspoints[accesspoints$size >= 100,] # Subset those which meet the size threshold (100ha) - n = 306 (1%)
temp$size <- NULL # Drop variables not required
write_parquet(osgsl_new, "../data/processed/osgsl/osgsl_district.parquet") # Save
